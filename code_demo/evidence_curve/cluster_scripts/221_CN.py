...
results_dir = str(Path().absolute()) + "/results/"

def injection_evidence(start_time):
    fit = qnm_filter.Network(segment_length=0.2, srate=4096 * 1, t_init=start_time*mass)
    ...
    _, evidence220 = qnm_filter.parallel_compute(fit, massspace, chispace, num_cpu = 4, model_list=[(2,2,0,'p')])
    ...
    contour220_221 = qnm_filter.find_credible_region(likelihood220_221, target_probability=0.95)
    contour220_221_330 = qnm_filter.find_credible_region(likelihood220_221_330, target_probability=0.95)
    occams220_221 = np.count_nonzero(likelihood220_221>contour220_221)/len(mass_grid.flatten())
    occams220_221_330 = np.count_nonzero(likelihood220_221_330>contour220_221_330)/len(mass_grid.flatten())
    nofilter = logsumexp(
        np.array(
    ...
    fit.original_data['H1'] = qnm_filter.RealData(signal220+signalH_noise, index=t_range, ifo="H1")
    fit.condition_data('pure_nr', remove_mean=False)
    fit.condition_data('original_data', remove_mean=False)
...
# Analysis
[t_range, signal, signal220, signal221, signal330, signalH_noise] = np.loadtxt("./realisations/220+221+330_realisation3.dat")
...
np.savetxt(
    str(Path().absolute()) + "/temp_results/220+221+330_realisation3_EC_"+str(time_space[index]) + ".dat",
    result)
